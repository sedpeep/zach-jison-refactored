...
describe("LALR Parser Tests", () => {
    describe("Basic Grammar Tests", () => {
        it("should parse 0+0 grammar correctly", () => {
            const lexData = {
                rules: [
                    ["0", "return 'ZERO';"],
                    ["\\+", "return 'PLUS';"]
        ...
        it("should handle xx nullable grammar", () => {
            const lexData = {
                rules: [
                    ["x", "return 'x';"]
                ]
            ...
            assert.ok(parser.parse("xxx"), "Should parse multiple x's");
    ...
    describe("Complex Grammar Tests", () => {
        it("should handle LALR algorithm from Bermudez, Logothetis", () => {
            const lexData = {
                rules: [
                    ["a", "return 'a';"],
                    ["b", "return 'b';"],
            ...
            assert.ok(parser.parse("agd"), "Should parse 'agd'");
            assert.ok(parser.parse("agc"), "Should parse 'agc'");
            assert.ok(parser.parse("bgd"), "Should parse 'bgd'");
            assert.ok(parser.parse("bgc"), "Should parse 'bgc'");
        });

        it("should parse basic JSON grammar", () => {
            const grammar = {
                "lex": {
                    "macros": {
                        "digit": "[0-9]",
            ...
            assert.ok(parser.parse(source), "Should parse complex JSON");
        });
    });

    describe("LR(1) Grammar Tests", () => {
        it("should detect conflicts in LR(1) grammar", () => {
            const grammar = {
            ...
            const gen = new Jison.Generator(grammar, {type: "lalr"});
            assert.equal(gen.conflicts, 2, "Grammar should have two conflicts");
        });
    });

    describe("BNF Grammar Tests", () => {
        it("should bootstrap BNF grammar parser", () => {
